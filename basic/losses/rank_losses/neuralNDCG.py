...
from basic.losses.rank_losses.loss_utils import deterministic_neural_sort, sinkhorn_scaling, stochastic_neural_sort
from basic.losses.rank_losses.loss_metrics import dcg
...
class NeuralNDCGLoss(nn.Module):
    def __init__(self, loss_weight=1.0, **kwargs):
        ...
        Args:
            ...
            padded_value_indicator (int, optional): an indicator of the y_true index containing a padded item, e.g. -1
            temperature (float, optional): temperature for the NeuralSort algorithm
            powered_relevancies: whether to apply 2^x - 1 gain function, x otherwise
            k: rank at which the loss is truncated
            stochastic: whether to calculate the stochastic variant
            n_samples: how many stochastic samples are taken, used if stochastic == True
            beta: beta parameter for NeuralSort algorithm, used if stochastic == True
            log_scores (bool, optional): log_scores parameter for NeuralSort algorithm, used if stochastic == True
        Returns:
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: loss value, a torch.Tensor
        """
        return self.loss_weight * (1 + neuralNDCG(y_pred, y_true, **self.kwargs))
