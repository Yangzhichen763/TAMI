...
from basic.metrics.util import reorder_image, to_y_channel
from basic.utils.registry import METRICS_REGISTRY

from .util import unpaired_reduce, _reduction_modes


'''
Modified from Retinexformer(https://github.com/caiyuanhao1998/Retinexformer/blob/master/basicsr/metrics/niqe.py)
'''
...
class NIQE:
    """
    Calculate NIQE (Natural Image Quality Evaluator) between two images.

    Args:
        reduction (str): Specifies the reduction to apply to the output.
            Supported choices are 'none' | 'mean' | 'sum'. Default: 'mean'.
        crop_border (int): Cropped pixels in each edge of an image. These pixels are not involved in the NIQE calculation.
    """
    metric_mode = 'NR'
    ...
    def __call__(self, pred):
        ...
        """
        return calculate_niqe(pred, reduction=self.reduction, **self.metrics_kwargs)
...
def calculate_niqe(img, crop_border=0, input_order='HWC', convert_to='y'):
    ...
    # we use the official params estimated from the pristine dataset.
    niqe_pris_params = np.load('basic/metrics/niqe_pris_params.npz')
