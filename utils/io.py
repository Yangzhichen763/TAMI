...
#region ==[Image File I/O]==
# Read images (supports .npy and common image formats). Adapted from FastLLVE utilities:
# https://github.com/Wenhao-Li-777/FastLLVE/blob/main/data/util.py (read_img/read_img2)
...
# Save images (supports .npy and common image output formats)
...
#region ==[Glob Path Utilities]==
# Load a list of file paths from a pattern.
# Adapted from FastLLVE utility:
# https://github.com/Wenhao-Li-777/FastLLVE/blob/main/data/util.py (glob_file_list)
'''
# Original:
...
# Recursively glob all matching files, sorted in natural order
...
# Recursively glob all matching files, sorted in natural order, and group them by subdirectory
def glob_packed_files(directory, file_extensions, path_handler=PathHandler.get_vanilla_path):
    ...
    # Convert dict values (lists) to a list
